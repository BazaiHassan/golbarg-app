...
export default defineEventHandler(async (event) => {
    try {
        ...
        for (const file of files) {
            ...
            try {
                // Save file to public directory using Nuxt's storage
                console.log(`[UPLOAD] Saving to public directory via storage`)
                ...
                // Fallback to assets:public:images if available
                try {
                    console.log(`[UPLOAD] Fallback: Saving to assets:public:images`)
                    await useStorage('assets:public:images').setItemRaw(uniqueFilename, file.data)
                    
                    const storageMethod = 'assets:public:images'
                    const storagePath = `assets/public:images/${uniqueFilename}`
                    ...
                    console.error(`[UPLOAD] ❌ assets:public:images failed:`, assetsError)
                    
                    // Final fallback to data storage
                    try {
                        console.log(`[UPLOAD] Final fallback: Saving to data storage`)
